...
//                matrix[t+1][i] = matrix[t][i] + (Const.tStep * (Const.a * Math.pow(matrix[t][i], 1/2)
//                        * ((Math.pow(matrix[t][i + 1] - matrix[t][i - 1], 2) / 8
//                        + matrix[t][i-1] - 2 * matrix[t][i] + matrix[t][i+1]) / Math.pow((Const.xStep+1), 2)) + Const.B));
            }
        }

        long endTime = System.currentTimeMillis();
        graphic.print("Sequential: " + (endTime - beginTime) + " ms \n");
