...
public class ParallelSolving {
     ...
     void calculate(){
          ...
          IntStream.range(1, Const.timeIterations).forEach( t -> {
               IntStream.range(1, Const.xIterations - 1).parallel().forEach(i -> {
                    matrix[t][i] = approximatedSolution(matrix[t-1][i-1], matrix[t-1][i], matrix[t-1][i+1],
                            Const.tStep, Const.xStep, Const.a, Const.b);
               });
          });
          long endTime = System.currentTimeMillis();
          graphic.print("Parallel: " + (endTime - beginTime) + " ms \n");
          Exact.calculate();
          graphic.printToPlot(matrix, Const.fileParallel);
